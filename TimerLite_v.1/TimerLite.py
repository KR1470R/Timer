...
RINGTONE = os.path.abspath('ringtones/ringtone.ogg')
...
def scroll_type(event):
    if event.num == 10 or event.delta == -120:
        return -1
...
class Timer(tk.Tk):
    def __init__(self):
        ...
        self.photo1 = os.path.abspath('Widgets_ icon/GREENBUTTON.png')
        self.photoimage1 = PhotoImage(file = self.photo1)
        self.photo2 = os.path.abspath('Widgets_ icon/PAUSEBUTTON.png')
        self.photoimage2 = PhotoImage(file = self.photo2)
        self.photo3 = os.path.abspath('Widgets_ icon/ADDBUTTON.png')
        self.photoimage3 = PhotoImage(file=self.photo3)

        self.button_add = tk.Label(self.button_frame, text='Add Timer', image= self.photoimage3)
        self.button_add.grid(column=0, row=0, sticky='WE')
        self.button_add.bind("<Button-1>", self.create_counter)

        self.button_start = tk.Label(self.button_frame, text='Start',image = self.photoimage1)
        self.button_start.grid(column=1, row=0, sticky='WE')
        self.button_start.bind("<Button-1>", self.start_all)

        self.button_stop = tk.Label(self.button_frame, text='Pause', image = self.photoimage2)
...
class Counter(object):
    def __init__(self, master):
        ...
        self.title_text.set('Text for reminder')
        self.title = tk.Entry(self.frame, font=(FONT_NAME, LABEL_SIZE), textvariable=self.title_text, bd=0)
        def clear_box(event):
            self.title.delete(0,END)
        self.title.bind('<Button-1>', clear_box)
    ...
    def tick(self):
        if not self.paused and self.time > 0:
            ...
            if self.time == 0:
                ...
                def message_box():
                    message = messagebox.askquestion('Time Left!', self.title.get()+'\n Continue?')
                    if message == 'yes':
                        mixer.init()
                        mixer.music.stop()
                    elif message == 'no':
                        os.kill(os.getpid(), signal.SIGKILL)
                t1 = Thread(target=ringtone)
                t1.start()
                t2 = Thread(target=message_box)
                t2.start()
        self.refresh()
...
def main():
    ...
    try:
        icon = os.path.abspath('ico/TimerLite.ico')
        app.iconbitmap(icon)
    except:
        pass
    app.mainloop()
